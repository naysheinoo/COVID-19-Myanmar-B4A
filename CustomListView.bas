...
Sub Class_Globals
	...
	Public defaultPresscolor As Int
...
Private Sub sv_ScrollChanged(Position As Int)
	If Position + sv.Height >= sv.Panel.Height - 400dip Then
...
Public Sub InsertAt(Index As Int, Pnl As Panel, ItemHeight As Int, Value As Object)
	...
	c.Initialize ( defaultPresscolor , 0)
'	Dim b As BitmapDrawable 
'	b.Initialize (LoadBitmap(File.DirAssets , "image/mpd_headbar.png"))
	
	Dim sd As StateListDrawable
    sd.Initialize
    sd.AddState(sd.State_Pressed, c )'pressedDrawable)
    sd.AddCatchAllState(c) 'Pnl.Background
